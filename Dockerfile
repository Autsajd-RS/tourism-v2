...
# composer
    && cd /usr/bin/ && wget -O composer https://getcomposer.org/download/2.3.2/composer.phar && chmod +x /usr/bin/composer \
# fix log path
    && sed -i "s/error_log.*/error_log = \/var\/log\/php8\.1\-fpm\.error.log/g" /usr/local/etc/php-fpm.d/docker.conf \
    && sed -i "s/access.log.*/access.log = \/var\/log\/php8\.1\-fpm\.access.log/g" /usr/local/etc/php-fpm.d/docker.conf \
    && ln -sf /dev/null /var/log/php8.1-fpm.access.log \
    && ln -sf /proc/1/fd/2 /var/log/php8.1-fpm.error.log
...
RUN apk add nginx \
    && mkdir -p /run/nginx
COPY services/configs/nginx/http.d/nginx.conf /etc/nginx/http.d/default.conf
